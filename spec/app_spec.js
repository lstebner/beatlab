...
(function() {
  ...
  describe("App.Router", function() {
    ...
    describe("#get", function() {
      return it("should register a new route as get", function() {
        var new_route;
        new_route = test_router.get("/whateverman", {}, "whateverman");
        expect(new_route[0]).to.be("get");
        return expect(new_route[1]).to.be("/whateverman");
      });
    });
    return describe("#post", function() {
      return it("should register a new route as post", function() {
        var new_route;
        new_route = test_router.post("/anchorman", {}, "anchorman");
        expect(new_route[0]).to.be("post");
        return expect(new_route[1]).to.be("/anchorman");
      });
    });
  });
  ...
  describe("App.View", function() {
    ...
    it("should autogenerate a unique id", function(done) {
      var id1;
    ...
    it("should be able to add to js_opts", function() {
      expect(test_view.js_opts).to.not.have.key("plum");
      test_view.add_js_opts({
        plum: "purple"
      });
      return expect(test_view.js_opts).to.have.key("plum");
    });
    it("should set the view", function() {
      test_view.set_view("blender");
      return expect(test_view.view).to.be("blender");
    });
    it("should be able to set some data", function() {
      expect(test_view.data).to.not.have.key("mangos");
      test_view.set_data("mangos", 5);
      expect(test_view.data).to.have.key("mangos");
      return expect(test_view.data.mangos).to.be(5);
    });
    it("should be able to extend existing data", function() {
      expect(test_view.data).to.not.have.keys("kiwi", "plantain");
      test_view.extend_data({
        kiwi: true,
        plantain: true
      });
      expect(test_view.data).to.have.keys("kiwi", "plantain");
      expect(test_view.data.kiwi).to.be(true);
      return expect(test_view.data.plantain).to.be(true);
    });
    return it("should not contain a js_block by default", function() {
      return expect(test_view.js_block()).to.be(false);
    });
  });
